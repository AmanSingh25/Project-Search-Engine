...
def title_to_info(metadata):
    ...
    for articles in metadata:
        dict_1['author'] = articles[1]
        dict_1['timestamp'] = articles[2]
        dict_1['length'] = articles[3]
        dict_2[articles[0]] = dict_1
        dict_1 = {}
    return dict_2
    ...
    for articles in metadata:
        for keywords in articles[4]:
            if keywords not in dict:
                dict[keywords] = [articles[0]]
            else:
                dict[keywords] = dict[keywords] + [articles[0]]
    return dict
...
def search(keyword):
    ...
    for keywords in key:
        if keywords.lower() == keyword.lower():
            return key[keywords]
    return []        
...
def article_info(titles, title_to_info):
    ...
    for articles in titles:
        if articles in title_to_info:
                dict[articles] = title_to_info[articles]
    return dict             
...
def article_length(article_length, titles, title_to_info):
    list = []
    for articles in titles:
        if title_to_info[articles]['length'] <= article_length:
...
def title_timestamp(titles, title_to_info):
    dict = {}
    for articles in titles:
        dict[articles] = title_to_info[articles]['timestamp']
    return dict        
